...
export class ImagesLoader {
  private readonly images: { [url: string]: HTMLImageElement };
  private readonly pendingActions: ((images: { [url: string]: HTMLImageElement }) => void)[] = [];
  ...
  constructor(urls: string[]) {
    ...
    for (const url of urls) {
      ...
      image.src = url;
      loader.images[url] = image;
    }
  }

  public invokeAfterLoaded(action: (images: { [url: string]: HTMLImageElement }) => void): void {
