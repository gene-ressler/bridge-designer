...
import { PointTag } from './site-constants';
...
export class SiteRenderingHelper2D {
  ...
  public static fillEarthProfileInfo(
    ...
    const xBaseLeft = siteInfo.xLeftmostDeckJoint - SiteConstants.ABUTMENT_INTERFACE_SETBACK;
    if (siteInfo.designConditions.isAtGrade) {
      ...
      for (const curvePt of SiteConstants.ACCESS_CURVE) {
        ...
        y = viewportTransform.worldToViewportY(SiteConstants.DECK_TOP_HEIGHT + curvePt.y);
    ...
    const xBaseRight = siteInfo.xRightmostDeckJoint + SiteConstants.ABUTMENT_INTERFACE_SETBACK;
    ...
    y = viewportTransform.worldToViewportY(
      SiteConstants.ELEVATION_TERRAIN_POINTS[siteInfo.rightAbutmentInterfaceTerrainIndex].y + siteInfo.yGradeLevel,
    );
    earthProfile.lineTo(x, y);
    ...
    x = viewportTransform.worldToViewportX(siteInfo.xLeftmostDeckJoint - SiteConstants.ABUTMENT_INTERFACE_SETBACK);
    earthProfile.lineTo(x, y);
  ...
  public static renderAbutmentsAndPier(
    ...
    if (conditions.isArch) {
      ...
      siteDetailRenderers.renderArchAbutment(pLeft, AbutmentSide.LEFT, archHeight, ctx, viewportTransform);
      siteDetailRenderers.renderArchAbutment(pRight, AbutmentSide.RIGHT, archHeight, ctx, viewportTransform);
    } else {
      ...
      siteDetailRenderers.renderStandardAbutment(pRight, AbutmentSide.RIGHT, 1, ctx, viewportTransform);
    }
  ...
  public static renderStandardAbutmentImpl(
    ...
    SiteRenderingHelper2D.renderPillow(ctx, location.x, location.y, viewportTransform);
  }
  ...
  public static renderArchAbutmentImpl(
    ...
    ctx.fillStyle = savedFillStyle;
    ctx.strokeStyle = savedStrokeStyle;
    SiteRenderingHelper2D.renderPillow(ctx, location.x, location.y + archHeight, viewportTransform);
  }
  ...
  private static renderAbutmentWearSurfaceImpl(
    ...
    const y = viewportTransform.worldToViewportY(location.y + SiteConstants.DECK_TOP_HEIGHT);
  ...
  private static renderPillow(
    ...
    ctx.beginPath();
    for (const p of SiteConstants.PILLOW_POINTS) {
      ctx.lineTo(viewportTransform.worldToViewportX(x + p.x), viewportTransform.worldToViewportY(y + p.y));
    }
    ctx.closePath();
