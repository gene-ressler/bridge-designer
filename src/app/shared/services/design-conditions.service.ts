...
const MAX_SLENDERNESS = 300;

/** Container for data and algorithms regarding the conditions imposed on a bridge design. */
export class DesignConditions {
  ...
  private constructor(tag: string, codeLong: number, contestParameters: ContestParameters) {
    this.tag = tag;
    this.codeLong = codeLong;
    const nullableCode = DesignConditions.getCode(codeLong);
    const error = DesignConditions.getCodeError(nullableCode);
    if (error != CodeError.NONE) {
      console.error('bad design conditions code (%d) in constructor %d', error, codeLong);
      return;
    }
    const code = nullableCode!;
    this.code = code;
    ...
    // Pre-computation of condition-dependent site geometry and design constraints.
    ...
    this.overMargin = SiteConstants.GAP_DEPTH + MIN_OVERHEAD - this.deckElevation;
    ...
    // Slenderness limit.
    this.allowableSlenderness = isLeftCable || isRightCable ? Number.POSITIVE_INFINITY : MAX_SLENDERNESS;

    // Cost calculations.
    this.excavationVolume = DesignConditions.getExcavationVolume(this.deckElevation);
    this.deckCostRate =
      this.deckType === DeckType.MEDIUM_STRENGTH
        ? contestParameters.deckCostPerPanelMedStrength
        : contestParameters.deckCostPerPanelHiStrength;
    ...
    if (this.isFromKeyCode) {
      ...
      if (this.isPier) {
        ...
        this.pierCost =
          ...
          this.excavationVolume * contestParameters.excavationCostRate -
          this.abutmentCost -
      ...
      this.abutmentCost = isArch
        ? this.panelCount * contestParameters.archIncrementalCostPerDeckPanel +
          DesignConditions.getArchAbutmentCost(this.underClearance)
        : this.isPier
          ? contestParameters.standardAbutmentBaseCost +
            Math.max(this.pierPanelIndex, this.panelCount - this.pierPanelIndex) *
              contestParameters.standardAbutmentCostPerDeckPanel
          : contestParameters.standardAbutmentBaseCost +
            this.panelCount * contestParameters.standardAbutmentCostPerDeckPanel;
      this.pierCost = this.isPier
        ? Math.max(this.pierPanelIndex, this.panelCount - this.pierPanelIndex) *
            contestParameters.pierCostPerDeckPanel +
          DesignConditions.getPierHeightCost(this.pierHeight) +
          contestParameters.pierBaseCost
        : 0;
      this.totalFixedCost =
        this.excavationVolume * contestParameters.excavationCostRate +
        this.abutmentCost +
    ...
    this.supportedJointIndices = isArch
      ? [this.archJointIndex, this.archJointIndex + 1]
      : this.isPier
    ...
    this.siteCosts = {
      ...
      excavationCostRate: contestParameters.excavationCostRate,
      ...
      anchorageCostPer: contestParameters.anchorageCostPer,
      anchorageCost: this.anchorageCount * contestParameters.anchorageCostPer,
  ...
  public get isFromKeyCode() {
    return this.tag === FROM_KEY_CODE_TAG;
  }
  ...
  private static getCodeError(code: Uint8Array | null): CodeError {
    ...
    if (deckElev + over > SiteConstants.GAP_DEPTH + MIN_OVERHEAD) {
...
export class DesignConditionsService {
  ...
  public static readonly PLACEHOLDER_CONDITIONS = DesignConditions._placeholderConditions;

  /** Pre-defined design conditions. Intent is that the list of tuples can be garbage-collected. */
  private readonly standardConditions: DesignConditions[];
  private readonly standardConditionsBySetupKey: Map<string, DesignConditions>;
  private readonly standardConditionsFromCode: Map<number, DesignConditions>;
  ...
  constructor(private readonly contestParametersService: ContestParametersService) {
    this.standardConditions = //#region data for standard conditions
      ...
      ).map(p => DesignConditions.fromTaggedCodeLong(...p, contestParametersService.parameters));

    this.standardConditionsBySetupKey = this.standardConditions.reduce((map, conditions) => {
      map.set(conditions.setupKey, conditions);
      return map;
    }, new Map<string, DesignConditions>());

    this.standardConditionsFromCode = this.standardConditions.reduce((map, conditions) => {
      map.set(conditions.codeLong, conditions);
      return map;
    }, new Map<number, DesignConditions>());

    this.standardConditionsFromTag = this.standardConditions.reduce((index, conditions) => {
  ...
  public getConditionsForSetupKey(key: string): DesignConditions {
    return Utility.assertNotUndefined(this.standardConditionsBySetupKey.get(key), 'design conditions for key');
  }

  /**
   * Tries to fetch conditions for the given code from tagged standards.
   * If none exists, builds a new one tagged as from key code. Returns undefined
   * if code is invalid.
   */
  public getConditionsForCodeLong(code: number): DesignConditions | undefined {
    const tagConditions = this.standardConditionsFromCode.get(code);
    if (tagConditions) {
  ...
  public isTagPrefix(s: string): boolean {
    let lo = 0;
    let hi = this.standardConditions.length - 1;
    while (lo < hi) {
      const mid = Math.floor((lo + hi) / 2);
      const midKey = this.standardConditions[mid].tag.substring(0, s.length);
      if (s < midKey) {
        hi = mid - 1;
      } else if (s > midKey) {
        lo = mid + 1;
